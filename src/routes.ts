...
export async function getTrips(req: Request, res: Response): Promise<void> {
  ...
  try {
    const response = await fetch(
      `https://z0qw1e7jpd.execute-api.eu-west-1.amazonaws.com/default/trips?destination=${queryParams.destination}&origin=${queryParams.origin}`,
      {
        method: "GET",
        headers: {
...
export async function addFavorite(req: Request, res: Response): Promise<void> {
  ...
  if (!username || !trip) {
    res.status(400).send("Username and trip details are required.");
    return;
  }
  ...
  try {
    ...
    if (favorites.trips.find((t: Trip) => t.id === trip.id)) {
      res
        .status(409)
        .send(`This trip is already in the favorites list for ${username}`);
    } else {
...
export async function getFavorites(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (!favorites || favorites.trips.length === 0) {
...
export async function removeFavorite(
  ...
  if (!username || !tripId) {
    res.status(400).send("Username and trip ID are required.");
    return;
  }
  ...
  try {
    ...
    const initialLength = favorites.trips.length;
    favorites.trips = favorites.trips.filter((t: Trip) => t.id !== tripId);

    if (initialLength === favorites.trips.length) {
      res.status(404).send("Trip not found in favorites.");
      return;
    }
